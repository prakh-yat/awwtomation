  ...
  return (
    <>
      ...
      <div className="space-y-6">
        {items.length > 0 ? (
          <div className="grid grid-cols-1 divide-y rounded-lg border bg-card shadow-card sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            <div className="px-5 py-4">
              <p className="text-xs text-muted-foreground">Links</p>
              <p className="mt-1 text-2xl font-semibold tracking-tight">{formatNumber(items.length)}</p>
            </div>
            <div className="px-5 py-4">
              <p className="text-xs text-muted-foreground">Clicks in the last 7 days</p>
              <p className="mt-1 text-2xl font-semibold tracking-tight">{formatNumber(totals.clicks7d)}</p>
            </div>
            <div className="px-5 py-4">
              <p className="text-xs text-muted-foreground">Clicks all time</p>
              <p className="mt-1 text-2xl font-semibold tracking-tight">{formatNumber(totals.clicks)}</p>
              <p className="mt-0.5 text-xs text-muted-foreground">Link previews generated by Instagram aren&apos;t counted.</p>
            </div>
          </div>
        ) : null}
        ...
        {items.length === 0 ? (
          <EmptyState
            icon={Link2}
            title="No links yet"
            description="Create a short link, add it to a message button and see how many people tap it, and who they are."
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="ml-auto text-xs text-muted-foreground">
                {visible.length === items.length
                  ? `${formatNumber(items.length)} links`
      ...
      <ConfirmDialog
        ...
        description={
          deleting
            ? `${deleting.shortUrl} stops working straight away, so anyone who taps it in an old message will see an error. Its ${formatNumber(deleting.clickCount)} recorded clicks are deleted too.`
            : undefined
        }
