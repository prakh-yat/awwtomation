...
- **Flow builder**: visual canvas (React Flow) — Trigger → Message (with buttons) → Follow gate → Delay → Tag.
- **Inbox**: unified live chat for IG + FB with the 24h window indicator.
- **Contacts**: everyone who interacted, with tags + custom fields.
- **Broadcasts**: send a message to a tagged audience (only contacts inside the 24h window are eligible — Meta rule).
- **Analytics**: DMs sent, triggers, CTR via tracked links, per automation and per workspace.
- **Tracked links**: `/l/{slug}` redirects with click counting.
- **Workspaces & team**: owner/admin/member roles, invite links.
- **Plans & usage**: FREE/STARTER/PRO/AGENCY with DM + channel caps, enforced server-side.
- **Platform admin**: super-admin panel (all workspaces, plan overrides, system health, job queue).
...
- **Tenant isolation is non-negotiable**: every Prisma query on tenant data must include `workspaceId` (or go through a relation that was itself loaded by workspaceId). Never trust ids from the client without scoping.
...
- Loading + empty states for every list. Skeletons for initial loads. Every page has an `<h1>` and a one-line description via `<PageHeader>`.
...
Auth: `/login` (Google button), `app/auth/callback/route.ts` (Supabase code exchange → upsert User → ensure workspace → redirect), `app/auth/signout/route.ts`.
App (protected, `app/(app)/`, uses sidebar shell):
- `/dashboard` — overview KPIs + charts + recent activity
- `/automations`, `/automations/new`, `/automations/[id]` (builder), `/automations/[id]/analytics`
...
- `/links`
- `/logs`
- `/settings` (general), `/settings/team`, `/settings/billing`
- `/onboarding` (no workspace yet → create; then connect a channel)
- `/admin`, `/admin/workspaces`, `/admin/workspaces/[id]`, `/admin/jobs`, `/admin/health` (super admin only)
Other: `/invite/[token]` (accept invite), `/l/[slug]` (tracked-link redirect, `route.ts`).
...
Active workspace is stored in cookie `or_workspace` (workspace id). Sidebar has a workspace switcher that sets it via `POST /api/workspaces/switch`.

## 5. Shared module contracts (exact exports — implement OR consume exactly these)

Already written (do not rewrite): `lib/env.ts` (`getEnv`, `optionalEnv`, `appUrl`, `superAdminEmails`, `isMetaConfigured`), `lib/db.ts` (`prisma`, re-exports `@prisma/client`), `lib/crypto.ts` (`encrypt`, `decrypt`, `sha256`, `hmacSha256`, `constantTimeEqual`, `randomToken`, `signState`, `verifyState`), `lib/utils.ts` (`cn`, `slugify`, `formatNumber`, `formatPercent`, `truncate`, `initials`, `sanitizeNextPath`), `lib/brand.ts`.
...
### lib/auth/session.ts
- `getCurrentUser(): Promise<User | null>` — reads Supabase session, upserts our `User` by `supabaseId` (email/name/avatar from Google metadata), sets `isSuperAdmin` when email ∈ `superAdminEmails()`. Cached per request with `React.cache`.
...
export type WorkspaceContext = {
  ...
  workspace: Workspace;
  role: WorkspaceRole;
  memberships: Array<{ workspace: Pick<Workspace,"id"|"name"|"slug"|"plan">; role: WorkspaceRole }>;
...
`reserveDmQuota(workspaceId, n = 1): Promise<{ ok: boolean; used: number; limit: number }>` — atomic; resets `dmsSentThisPeriod` when `usagePeriodStart` is before the current month. `getUsage(workspaceId)`. `canAddChannel(workspaceId)`, `canAddAutomation(workspaceId)`, `canAddMember(workspaceId)`.
...
`channels.ts`, `automations.ts`, `contacts.ts`, `inbox.ts`, `broadcasts.ts`, `analytics.ts`, `links.ts`, `logs.ts`, `workspaces.ts`, `admin.ts`, `templates.ts` (static flow templates: "Link in DM", "Follow to unlock", "Lead magnet + tag", "Story reply → link", "Giveaway entry").
...
- Sidebar: 16rem, `bg-sidebar`, border-right, logo top, nav groups (Main: Dashboard, Automations, Inbox, Contacts, Broadcasts · Setup: Channels, Links, Logs · Settings), workspace switcher, user menu bottom. Collapsible to icon rail (cookie `or_sidebar=collapsed`). Active item: `bg-sidebar-accent` + black text; inactive: gray text.
...
`NEXT_PUBLIC_APP_URL`, `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `DATABASE_URL`, `DIRECT_URL`, `APP_ENCRYPTION_KEY`, `CRON_SECRET`, `META_APP_ID`, `META_APP_SECRET`, `INSTAGRAM_APP_ID`, `INSTAGRAM_APP_SECRET`, `META_WEBHOOK_VERIFY_TOKEN`, `META_GRAPH_API_VERSION`, `SUPER_ADMIN_EMAILS`, optional `RESEND_API_KEY`, `EMAIL_FROM`.
...
| Lane | Owns |
|---|---|
| foundation-auth | `lib/logger.ts`, `lib/supabase/*`, `lib/auth/*`, `lib/workspace/*`, `lib/billing/*`, `lib/services/workspaces.ts`, `middleware.ts`, `app/auth/*`, `app/login/*`, `app/api/workspaces/*`, `app/(app)/onboarding/*`, `app/invite/[token]/*`, `app/api/invitations/*` |
...
| automations | `app/(app)/automations/*`, `components/automations/*`, `lib/services/automations.ts`, `lib/services/templates.ts`, `app/api/automations/*` |
| inbox | `app/(app)/inbox/*`, `components/inbox/*`, `lib/services/inbox.ts`, `app/api/inbox/*` |
| contacts | `app/(app)/contacts/*`, `components/contacts/*`, `lib/services/contacts.ts`, `app/api/contacts/*` |
...
| admin | `app/(app)/admin/*`, `components/admin/*`, `lib/services/admin.ts`, `app/api/admin/*` |
