...
- **Flow builder**: visual canvas (React Flow) — Trigger → Message (with buttons) → Ask a question → Follow gate → Delay → Tag → Add to pipeline / Move stage / Remove from pipeline. "New automation" opens an empty canvas (just the trigger); templates live at `/automations/templates`.
...
- `/onboarding` (no organization yet → create one with its first workspace; then connect a channel)
...
`channels.ts`, `automations.ts`, `contacts.ts`, `pipelines.ts` (pipelines, stages and contact moves; every move writes an AuditLog `contact.stage_changed` or `contact.pipeline_removed`), `segments.ts`, `inbox.ts`, `broadcasts.ts`, `analytics.ts`, `links.ts`, `logs.ts`, `workspaces.ts`, `templates.ts` (static flow templates: "Link in DM", "Follow to unlock", "Lead magnet + tag", "Story reply → link", "Giveaway entry"). Organization-level services take `organizationId` first: `organizations.ts` (create, switch, rename, delete, members, ownership, invitations), `billing.ts`, `usage-history.ts`. `audit.ts` exports `recordAudit`.
