...
- **Tracked links**: `/l/{slug}` redirects with click counting. Bots, link scanners, email security gateways and prefetches are not counted, and the same visitor clicking again within 30 minutes counts once.
...
- **Plans & usage**: STARTER/PRO/AGENCY per organization; there is no free tier. An organization without a subscription is on NONE: it can connect an account, build automations and try its AI agents, but nothing is sent (automations cannot be switched on, flows do not start, DMs are 0) until it picks a plan. Each plan has DM, channel, workspace, automation, contact, broadcast and seat caps counted across all of its workspaces, AI agent and pipeline caps per workspace, all enforced server-side. At the contact limit nothing creates a contact: a new person who comments or messages is not saved (no contact, no inbox thread; a matching automation logs SKIPPED_CONTACT_LIMIT), and imports and manual adds stop. Contacts already past the limit after a downgrade stay, but automations skip the newest of them.
- **Retention**: each plan keeps a history window (Starter 90 days, Pro 180, Agency 365; NONE 30). Delivery logs and conversation messages older than that are deleted by housekeeping (`lib/services/retention.ts`), never inside the current billing month, and a lapsed or cancelled subscription keeps its paid window for 30 more days. Processed webhook receipts go after 15 days (unprocessed ones after 30), failed and cancelled jobs after 15, completed jobs after 7, audit logs after a year. Flow sessions waiting for a reply stop capturing messages after 24 hours and any session idle for 7 days stops resuming; housekeeping marks them EXPIRED. Analytics, the dashboard periods and the automation report never reach past the window. Contacts, conversations, automations, flow sessions and broadcast totals stay; "once per contact" reads `AutomationRecipient` and past usage months read `UsageMonthTotal`, both kept.
...
- **Product tour**: coach marks over the app shell (`components/app-shell/product-tour.tsx`, anchored on `data-tour` attributes), shown once per person per version of the tour (`User.tourVersion` against `TOUR_VERSION` in `lib/services/tour.ts`, set by a server action in `app/(app)/(shell)/tour-actions.ts`; raising the version shows a reworked tour to everyone once). It runs on the dashboard: the Connect button first, then the dock and every section tile in the dock's order, the workspace, usage and account slots, and a closing card. Each card leads with a band in the colour of what it points at, the progress is the steps' own colours, the dock magnifies each tile as its step comes up, and a step with somewhere to go offers it ("Connect now" opens the Connect menu, "Open Inbox", "Browse templates"), which ends the tour. It opens by itself the first time a person reaches the dashboard, and "Take the tour" in the account menu goes to the dashboard and replays it. It holds the dock open for its dock steps and uses the menu button on phones.
...
- AI replies: `lib/ai/*` (providers, prompt assembly with `PLATFORM_RULES`, marker parsing, `builtin.ts` for the built-in model, `presets.ts` for the provider picker, logos in `public/providers/`), `lib/services/ai.ts` (CRUD, `runAgent`, live model lists), the `ai_reply` flow node, and `/ai` for agents and providers. An agent with no connection (`providerId` null) replies with the built-in model on our key (`DEFAULT_AI_*`, OpenRouter by default, reply length capped, playground rate limited; the UI and MCP only ever name it "Awwtomation AI", never the model behind it); a workspace can connect its own key instead. A connection has a default model; an agent on a connection may name its own (`AiAgent.model`). The platform rules (English or Romanized Nepali only, short plain DMs, nothing invented, safety, markers) apply to both, and a reply in another script is rewritten once, then replaced by the fallback. Every agent is held to `lib/ai/limits.ts`: instructions 1,000 characters, knowledge 2,000, rules 1,000, the failure reply 500, reply length 60 to 400 tokens (new agents 200) and context 2 to 20 messages (new agents 10), and new agents start at creativity 0.4; the editor counts against them and will not save past them, and the API and MCP refuse the same. Knowledge is small enough to sit whole in every prompt, inside `<knowledge>` tags, and the platform rules make it and the instructions the only source of facts: no retrieval step, so nothing relevant is ever missed. The prompt also carries the business's current date and time in the workspace's time zone. `runAgent` retries a rate limit, outage, timeout or empty answer once (the built-in model on `DEFAULT_AI_FALLBACK_MODEL` when set), then checks the reply: a non-Latin script or a link the workspace never wrote gets one rewrite naming every problem; a script that survives it fails to the fallback reply, and a link that survives it is removed with its sentence. Reasoning a model leaks into its answer, role labels, markdown and a sentence cut off at the token limit are cleaned out. A reply that only closes the conversation sends nothing, and one that only hands over sends the fallback reply. `lib/ai/providers.ts` adapts parameters per model family (no `temperature` for reasoning models, `max_completion_tokens` for OpenAI, headroom for thinking) and retries once when a provider names a parameter it refuses.
...
App (protected, `app/(app)/`). Every product page is in the `(shell)` route group, whose layout draws the dock, the phone header and the tour; `/onboarding` and `/welcome` sit beside it and take the whole window. Keep it that way: a layout is not rendered again on a client-side navigation, so a single layout that chose between the shell and a bare page from the request path kept the wrong one after the welcome flow.
- `/dashboard`: overview KPIs, charts and recent activity, fitted to one screen; also where accounts are connected and managed (`?accounts=1` opens the accounts dialog; the Meta OAuth callbacks land here with `?connected=` or `?error=`; a newly connected account is asked three short questions in its platform's colour (what it is, how it makes money, what it should do first; the goals are only those its platform has templates for, so a Facebook Page is not offered follower growth), stored on `Channel.setupAnswers` and editable from "Edit details" in the account's menu; `PATCH /api/channels/[id]/setup`, MCP `update_account_details`)
- `/automations` (the template dialog opens over it on `?templates=1`, preselected with `&goal=` and `&platform=`), `/automations/[id]` (builder; draws edge to edge), `/automations/[id]/analytics`. `/automations/templates` and `/automations/new` redirect to the list with the dialog open.
...
- `/onboarding` (no organization yet → create one with its first workspace), then `/welcome` (the person's role and how they heard about us, asked once per person and stored on `User.profileAnswers`; `User.profileCompletedAt` records that they were answered or skipped, and until it is set the `(shell)` layout sends the person here from any page, with `?next=` to bring them back, which is how existing users and invited members get them too; who the workspace is for, stored on `Workspace.onboardingAnswers`; then, for an owner with no plan, the plan step with "Decide later"; it never asks to connect an account; full window, no dock, skippable)
...
API: `app/api/**` (see file ownership §9). Webhooks: `app/api/webhooks/meta/route.ts` (GET verify, POST receive). Cron: `app/api/cron/{tick,refresh-tokens,reconcile,housekeeping}/route.ts` guarded by `Authorization: Bearer ${CRON_SECRET}`. Queue health for an uptime monitor: `GET /api/health/queue` (200, or 503 once a due job has waited longer than `QUEUE_ALERT_AFTER_SECONDS`; full stats with the cron secret).
...
```ts
export type PlanLimits = { channels; workspaces; automations; dmsPerMonth; contacts; members; broadcastsPerMonth; aiAgentsPerWorkspace; pipelinesPerWorkspace; historyDays; priceUsd; priceAnnualUsd; label; description; features };
export const PLANS: Record<PlanTier, PlanLimits>;
//            channels workspaces automations DMs/mo   contacts  members broadcasts/mo AI agents/ws pipelines/ws history  price
// NONE        1        1          3          0        1_000     1        0            1            1            30 days   not sold (no plan yet: builds, sends nothing)
// STARTER     3        3          20         2_000    10_000    3        20           3            3            90 days   $15
// PRO         10       10         100        15_000   50_000    10       100          10           10           180 days  $49
// AGENCY      50       50         1_000      100_000  250_000   50       500          25           20           365 days  $149
export function limitsFor(plan: PlanTier): PlanLimits;
export function historyLabel(days: number): string; // "60 days", "1 year"
```
### lib/billing/usage.ts
`reserveDmQuota(workspaceId, n = 1): Promise<{ ok: boolean; used: number; limit: number }>`, atomic on the workspace's **organization** row; resets `dmsSentThisPeriod` when `usagePeriodStart` is before the current month. `getOrganizationUsage(organizationId)` / `getUsage(workspaceId)`, `checkOrganizationLimit(organizationId, kind)` / `checkLimit(workspaceId, kind)` for `channels | workspaces | automations | contacts | members`, `checkWorkspaceLimit(workspaceId, "aiAgentsPerWorkspace" | "pipelinesPerWorkspace")`, `checkBroadcastQuota(workspaceId, { scheduledFor?, excludeId? })` (broadcasts started this UTC month), `contactRoom(workspaceId)`, `contactWithinPlan(contact)` (the oldest contacts are the ones inside the limit), `historyDaysFor(workspaceId)`, `historyCutoff(historyDays)`, `canAddChannel(workspaceId)`, `canAddAutomation(workspaceId)`, `canAddMember(workspaceId)`. Organization counts span every workspace in it. `historyPlan(org)` in `entitlements.ts` picks the plan whose history window applies.
...
export async function claimJobsByIds(workerId: string, ids: readonly string[]): Promise<Job[]>; // claim particular due jobs (the webhook route runs its own jobs right after answering)
export async function runJob(job: Job): Promise<"completed" | "failed">; // handler, then complete or fail with backoff; never throws
export async function processBatch(workerId: string, limit: number): Promise<{ processed: number; failed: number; claimed: number }>; // claim → run all → for round-based callers (cron tick)
export async function getQueueStats(): Promise<{ pending; processing; due; oldestDueSeconds; failedLastDay }>; // live part of the table only
```
### lib/queue/handlers/index.ts
`export const handlers: Record<JobType, (job: Job) => Promise<void>>`, `EXECUTE_FLOW` → `lib/automation/engine.executeFlowStep`, `PUBLIC_REPLY` → `lib/automation/public-reply.sendPublicReply`, `BROADCAST_SEND` → `lib/services/broadcasts.sendBroadcastMessage`, `REFRESH_TOKEN` → `lib/meta/tokens.refreshChannelTokenIfNeeded`, `RECONCILE_COMMENTS` → `lib/automation/reconcile.reconcileChannel`, `SYNC_MEDIA` → `lib/services/channels.syncChannelMedia`, `PROCESS_WEBHOOK` → `lib/webhooks/processor.processWebhookJob`, `BROADCAST_FANOUT` → `lib/services/broadcasts.fanOutBroadcast`.

### worker/index.ts
A continuous pool: up to `WORKER_BATCH_SIZE` jobs run at once, and a finished job's slot is refilled straight away (also every `WORKER_POLL_INTERVAL_MS`), so one slow AI call never holds up the rest. `releaseStaleJobs()` every minute; a heartbeat every 5 minutes that also runs `checkQueueHealth()` (`lib/services/ops-alerts.ts`: logs, and emails `OPS_ALERT_EMAIL` once per stall); enqueue `RECONCILE_COMMENTS` per ACTIVE channel every `COMMENT_POLL_INTERVAL_MS`; enqueue `REFRESH_TOKEN` daily for Instagram channels expiring within 10 days and for every ACTIVE Facebook Page (a validity check); a token that fails emails the organization's owners and admins once (`lib/services/channel-alerts.ts`, `Channel.alertSentAt`). Comment polling runs every `COMMENT_POLL_INTERVAL_MS` (15 minutes), stops paging at its lookback, refreshes the Media cache after 6 hours and pauses a channel for 30 minutes when Meta reports app usage at 80% or more (`metaFetch`'s `onUsage`, `Channel.pollBackoffUntil`). Broadcasts fan out in `BROADCAST_FANOUT` pages of 1000 contacts instead of inside the request; `runHousekeeping()` (`lib/services/retention.ts`) every 6 hours, which `/api/cron/housekeeping` also runs daily for deployments without a worker. Graceful SIGTERM. Loads `.env` via `dotenv/config`.
...
- Webhook: verify `X-Hub-Signature-256` with `META_APP_SECRET` (Instagram Login webhooks are signed with `INSTAGRAM_APP_SECRET`, try both). Verify the signature first (only unsigned traffic is rate limited, in memory), store each comment, message and postback as a `WebhookEvent` (dedupe on its key), queue one `PROCESS_WEBHOOK` job per event and answer 200 (500 only when an event could not be stored, so Meta sends it again). The route runs its own jobs in `after()`; anything left is picked up by the worker or the cron tick. The job and the reconcile poll both take a lease on the row (`lib/webhooks/events.ts`), so an event is never handled twice, and a failed one is retried with backoff. Facebook feed likes, reactions and shares, comment edits and removals, and unsent DMs make no event at all.
...
| foundation-ui | `components/ui/*`, `components/app-shell/*`, `app/layout.tsx`, `app/(app)/layout.tsx`, `app/(app)/(shell)/layout.tsx`, `app/(marketing)/*`, `app/not-found.tsx`, `app/error.tsx`, `public/*` (logo svg, favicon), `components/marketing/*` |
| dashboard | `app/(app)/(shell)/dashboard/*`, `components/dashboard/*`, `lib/services/analytics.ts`, `app/api/analytics/*` |
| channels | `app/(app)/(shell)/channels/*`, `components/channels/*`, `lib/services/channels.ts`, `app/api/channels/*`, `app/api/meta/*`, `app/api/media/*` |
| automations | `app/(app)/(shell)/automations/*`, `components/automations/*`, `lib/services/automations.ts`, `lib/services/templates/*`, `app/api/automations/*` |
| inbox | `app/(app)/(shell)/inbox/*`, `components/inbox/*`, `lib/services/inbox.ts`, `app/api/inbox/*` |
| contacts | `app/(app)/(shell)/contacts/*`, `components/contacts/*`, `components/pipelines/*`, `lib/services/contacts.ts`, `lib/services/pipelines.ts`, `lib/pipelines/*`, `app/api/contacts/*`, `app/api/pipelines/*` |
| broadcasts | `app/(app)/(shell)/broadcasts/*`, `components/broadcasts/*`, `lib/services/broadcasts.ts`, `app/api/broadcasts/*` |
| settings | `app/(app)/(shell)/settings/*`, `components/settings/*` |
...
| links-logs | `app/l/[slug]/route.ts`, `app/(app)/(shell)/links/*`, `app/(app)/(shell)/logs/*`, `lib/services/links.ts`, `lib/services/logs.ts`, `app/api/links/*`, `app/api/logs/*` |
