...
- `/settings` (workspace and organization, ownership and deletion), `/settings/workspaces`, `/settings/mcp` (the MCP URL, per-app setup, connected apps, and every tool in three columns with who may use it: the owner edits that, everyone else reads it; every role), `/settings/team`, `/settings/billing`. `/settings/usage` and `/settings/pipeline` redirect to `/usage` and `/contacts/pipelines`.
...
- `lib/services/mcp-access.ts`: grants (`OAuthGrant`: one person, one app, one organization, all workspaces or a list). `consentOptions`, `saveConsent`, `reachableWorkspaces` (resolved against current memberships on every request), `hasLiveGrant`, `listConnectedApps`, `revokeGrant` (own app for anyone, anyone's for ADMIN+). Tool access (`McpToolAccess`, one row per organization and tool): no row means every member as their role allows; a row narrows the tool to `userIds`, and people who join later are not in it. `toolAccessFor`, `listToolAccess`, `setToolAccess`.
