...
- AI replies: `lib/ai/*` (providers, prompt assembly with `PLATFORM_RULES`, marker parsing, `builtin.ts` for the built-in model, `presets.ts` for the provider picker, logos in `public/providers/`), `lib/services/ai.ts` (CRUD, `runAgent`, live model lists), the `ai_reply` flow node, and `/ai` for agents and providers. An agent with no connection (`providerId` null) replies with the built-in model on our key (`DEFAULT_AI_*`, OpenRouter by default, reply length capped, playground rate limited); a workspace can connect its own key instead. A connection has a default model; an agent on a connection may name its own (`AiAgent.model`). The platform rules (English or Romanized Nepali only, short plain DMs, nothing invented, safety, markers) apply to both, and a reply in another script is rewritten once, then replaced by the fallback. `lib/ai/providers.ts` adapts parameters per model family (no `temperature` for reasoning models, `max_completion_tokens` for OpenAI, headroom for thinking) and retries once when a provider names a parameter it refuses.
...
- `/dashboard`: overview KPIs, charts and recent activity, fitted to one screen; also where accounts are connected and managed (`?accounts=1` opens the accounts dialog; the Meta OAuth callbacks land here with `?connected=` or `?error=`; a newly connected account is asked three short questions (what it is, how it makes money, what it should do first), stored on `Channel.setupAnswers` and editable from "Edit details" in the account's menu; `PATCH /api/channels/[id]/setup`, MCP `update_account_details`)
...
- `/onboarding` (no organization yet → create one with its first workspace), then `/welcome` (who the workspace is for, then, for an owner with no plan, the plan step with "Decide later"; it never asks to connect an account; full window, no dock, skippable, stored on `Workspace.onboardingAnswers`)
