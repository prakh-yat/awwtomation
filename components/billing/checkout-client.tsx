 ...
 */
export function CheckoutClient({ tier, initialInterval, email, defaultName, workspaceName, mode }: CheckoutClientProps) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!country) {
      setNotice({ kind: "error", title: "Choose your billing country.", body: "Dodo uses it to work out tax." });
      return;
    }
    setNotice(null);
    setSubmitting(true);
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-[minmax(0,1fr)_minmax(320px,440px)]">
      <div className="flex flex-col px-6 py-8 sm:px-10 lg:px-16 lg:py-12">
        ...
        <div className="mt-6 max-w-md">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Upgrading <span className="font-medium text-foreground">{workspaceName}</span> to {plan.label}. You&apos;ll add
            your card in Dodo&apos;s secure window next — nothing is charged until you confirm there.
          </p>
          ...
          <ul className="mt-6 grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">
            <li className="flex items-start gap-1.5">
              ...
              <span>Card handled by Dodo Payments; we never see it</span>
            </li>
            <li className="flex items-start gap-1.5">
              <ShieldCheck className="mt-0.5 h-3.5 w-3.5 shrink-0" />
              <span>Secure 3-D checkout</span>
            </li>
            <li className="flex items-start gap-1.5">
      ...
      <div className="border-t bg-muted/40 px-6 py-8 sm:px-10 lg:border-l lg:border-t-0 lg:px-10 lg:py-12">
